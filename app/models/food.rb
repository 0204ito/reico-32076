class Food < ApplicationRecord
  ...
  def self.change(change)
    case change
    when 'purchase_new'
      Food.all.order(purchase_date: :DESC)
    when 'purchase_old'
      Food.all.order(purchase_date: :ASC)
    when 'sell_new'
      # return Food.where.not(sell_by: nil).order(sell_by: "DESC")賞味期限を書いてないものは含めたくない場合
      Food.order('sell_by IS NULL, sell_by DESC')
    when 'sell_old'
      Food.order('sell_by IS NULL, sell_by ASC')
    when 'checked_false'
      Food.where.not(checked: true)
    when 'checked_true'
      Food.where.not(checked: false)
    end
  end
end
